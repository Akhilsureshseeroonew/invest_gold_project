...
(function () {
  ...
  var money = function (n) { return '₹' + inr.format(Math.max(0, Math.round(n))); };
  ...
  (function theme() {
    ...
    btn.addEventListener('click', function () {
      ...
      if (meta) meta.setAttribute('content', next === 'light' ? '#FCFAF5' : '#0A0F2C');
      try { localStorage.setItem('ig-theme', next); } catch (e) {}
    });
  })();
  ...
  (function nav() {
    ...
    var spyLinks = links.filter(function (a) { return a.getAttribute('href').charAt(0) === '#'; });
    ...
    var onScroll = function () {
      ...
      var pos = window.scrollY + window.innerHeight * 0.32, current = sections[0];
  ...
  (function counters() {
    ...
    var run = function (el) {
      ...
      if (reduceMotion) { el.textContent = inr.format(target); return; }
  ...
  (function calculator() {
    ...
    var el = {
      paneGold: $('#paneGold'), paneCash: $('#paneCash'),
      weight: $('#gWeight'), weightOut: $('#gWeightOut'),
      amount: $('#cAmount'), amountOut: $('#cAmountOut'),
      purity: $('#gPurity'), rate: $('#gRate'),
      tenure: $('#gTenure'), tenureOut: $('#gTenureOut'),
      interest: $('#gInterest'), interestOut: $('#gInterestOut'),
      label: $('#resultLabel'), amountOutBig: $('#resultAmount'),
      metaA: $('#metaA'), metaALabel: $('#metaALabel'), metaB: $('#metaB'), metaC: $('#metaC'),
      bubble: $('#calcBubble')
    };
    ...
    function compute() {
      ...
      el.interestOut.textContent = interest.toFixed(2).replace(/\.00$/, '') + '%';
    ...
    ['input', 'change'].forEach(function (evt) {
      [el.weight, el.amount, el.purity, el.rate, el.tenure, el.interest].forEach(function (i) {
  ...
  (function testimonials() {
    var root = $('#tcar'), track = $('#tcarTrack'), dotsWrap = $('#tcarDots');
    if (!root || !track) return;

    var originals = $$('.tslide', track);
    var total = originals.length;
    var index = 0, perView = 3, offset = 1, timer = null, animating = false;

    function perViewFor(w) { return w > 1024 ? 3 : (w > 760 ? 2 : 1); }
    ...
    function build() {
      ...
      for (var i = total - perView; i < total; i++) track.appendChild(originals[(i + total) % total].cloneNode(true));
      originals.forEach(function (s) { track.appendChild(s.cloneNode(true)); });
      for (var j = 0; j < perView; j++) track.appendChild(originals[j].cloneNode(true));
      position(false);
      paintDots();
    }

    function slides() { return $$('.tslide', track); }

    function position(animate) {
      var trackIndex = index + perView;
      track.style.transition = animate ? '' : 'none';
      track.style.transform = 'translateX(' + (-(trackIndex - offset) * (100 / perView)) + '%)';
      if (!animate) { void track.offsetWidth; track.style.transition = ''; }
      slides().forEach(function (s, i) { s.classList.toggle('is-active', i === trackIndex); });
      $$('button', dotsWrap).forEach(function (d, i) {
        d.setAttribute('aria-current', String(i === ((index % total) + total) % total));
      });
    }
    ...
    function go(dir) {
      ...
      window.setTimeout(function () {
        if (index >= total) { index -= total; position(false); }
        else if (index < 0) { index += total; position(false); }
        animating = false;
      }, reduceMotion ? 0 : 700);
    }

    function jump(i) { if (!animating) { index = i; position(true); } }
    ...
    function paintDots() {
      ...
      originals.forEach(function (_, i) {
        ...
        b.addEventListener('click', function () { jump(i); restart(); });
        dotsWrap.appendChild(b);
      });
    }

    function start() { if (!timer && !reduceMotion) timer = window.setInterval(function () { go(1); }, 5000); }
    function stop()  { window.clearInterval(timer); timer = null; }
    function restart() { stop(); start(); }

    $('.tcar__nav--prev', root).addEventListener('click', function () { go(-1); restart(); });
    $('.tcar__nav--next', root).addEventListener('click', function () { go(1);  restart(); });
    ...
    vp.addEventListener('pointerdown', function (e) { startX = e.clientX; stop(); });
    ...
    vp.addEventListener('pointercancel', function () { startX = null; start(); });
  ...
  (function phones() {
    var stage = $('#phones');
    if (!stage) return;
    var items = $$('.phone', stage), n = items.length, active = 0;
    ...
    var timer = window.setInterval(function () { active = (active + 1) % n; paint(); }, 3200);
    stage.addEventListener('mouseenter', function () { window.clearInterval(timer); });
    items.forEach(function (p, i) {
      p.addEventListener('click', function () { active = i; paint(); });
    });
  })();
  ...
  (function form() {
    ...
    var rules = {
      ...
      fPhone:   function (v) { return /^[6-9]\d{9}$/.test(v.replace(/[^\d]/g, '').replace(/^91(?=\d{10}$)/, '')); },
      fEmail:   function (v) { return /^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(v.trim()); },
      fService: function (v) { return v !== ''; }
    };
    ...
    form.addEventListener('submit', function (e) {
      ...
      // No backend wired yet — swap this for your POST endpoint.
      ok.classList.add('is-shown');
      form.reset();
      window.setTimeout(function () { ok.classList.remove('is-shown'); }, 8000);
    });
  })();

  /* ---------- 12. FLOATING MASCOT ASSISTANT ---------- */
  (function fab() {
    var fab = $('#fab'), btn = $('#fabBtn');
    if (!fab) return;
    ...
    if (hoverable) {
      fab.addEventListener('mouseenter', function () { open(true); });
      fab.addEventListener('mouseleave', function () { open(false); });
    }
    btn.addEventListener('click', function () { open(!fab.classList.contains('is-open')); });
    fab.addEventListener('focusin', function () { open(true); });
    document.addEventListener('click', function (e) { if (!fab.contains(e.target)) open(false); });
    $$('.fab__item', fab).forEach(function (a) { a.addEventListener('click', function () { open(false); }); });
  })();

  /* ---------- 13. BRANCH SEARCH ---------- */
  (function branches() {
    var input = $('#branchSearch'), list = $('#branchList'), empty = $('#branchEmpty');
    ...
    input.addEventListener('input', function () {
      ...
      cards.forEach(function (card) {
        var hay = (card.getAttribute('data-branch') + ' ' + card.textContent).toLowerCase();
  ...
  (function servicePrefill() {
    ...
    var exists = $$('option', sel).some(function (o) { return o.value === want; });
    if (!exists) return;
    sel.value = want;
    var field = sel.closest('.field');
    if (field) field.classList.remove('is-invalid');
    var form = $('#enquiryForm');
    if (form) window.setTimeout(function () {
  ...
  (function gallery() {
    var root = $('#gallery');
    if (!root) return;
    var items = $$('.gallery__item', root), dots = $('.gallery__dots', root), i = 0, timer = null;
    ...
    function go(d) { i = (i + d + items.length) % items.length; paint(); }
    function start() { if (!timer && !reduceMotion) timer = window.setInterval(function () { go(1); }, 5000); }
    function stop() { window.clearInterval(timer); timer = null; }
    function restart() { stop(); start(); }
    ...
    items.forEach(function (_, k) {
      ...
      b.addEventListener('click', function () { i = k; paint(); restart(); });
      dots.appendChild(b);
    });

    $('.gallery__nav--prev', root).addEventListener('click', function () { go(-1); restart(); });
    $('.gallery__nav--next', root).addEventListener('click', function () { go(1); restart(); });
    root.addEventListener('mouseenter', stop);
    root.addEventListener('mouseleave', start);
    document.addEventListener('visibilitychange', function () { document.hidden ? stop() : start(); });
  ...
  (function modals() {
    ...
    modals.forEach(function (m) {
      $$('[data-modal-close]', m).forEach(function (b) { b.addEventListener('click', function () { close(m); }); });
  ...
  (function applyForm() {
    var form = $('#applyForm');
    if (!form) return;
    var cv = $('#aCv'), cvName = $('#aCvName'), ok = $('#applyOk');
    var MAX = 5 * 1024 * 1024;
    var placeholder = cvName.textContent;

    cv.addEventListener('change', function () {
      var f = cv.files && cv.files[0];
      cvName.textContent = f ? f.name + ' · ' + Math.max(1, Math.round(f.size / 1024)) + ' KB' : placeholder;
      cv.closest('.field').classList.remove('is-invalid');
    });

    var rules = {
      aName:  function (v) { return v.trim().length >= 2; },
      aEmail: function (v) { return /^[^\s@]+@[^\s@]+\.[a-z]{2,}$/i.test(v.trim()); },
      aPhone: function (v) { return /^[6-9]\d{9}$/.test(v.replace(/[^\d]/g, '').replace(/^91(?=\d{10}$)/, '')); }
    };
    ...
    form.addEventListener('submit', function (e) {
      ...
      // No backend wired yet — swap this for your POST endpoint (multipart/form-data).
      ok.classList.add('is-shown');
      form.reset();
